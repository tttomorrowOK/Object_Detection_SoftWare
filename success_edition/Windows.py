...
class MY_GUI():
    def __init__(self, init_window_name):
        ...
        #标签
        self.init_data_label = Label(self.init_window_name, text="输入视频")
        ...
        self.log_label.grid(row=68, column=0)
        ...
        self.log_data = Text(self.init_window_name, width=66, height=9)  # 日志框
        self.log_data.grid(row=5, column=0, columnspan=10)
        ...
        self.todetect_button = Button(self.init_window_name, text="打开视频", bg="lightblue", width=10,
                                              command=self.chooice_doc)  # 调用内部方法  加()为直接调用
        self.todetect_button.grid(row=2, column=11)
        self.todetect_button = Button(self.init_window_name, text="播放分析视频", bg="lightblue", width=10,
    ...
    def chooice_doc(self):
        ...
        self.video_path = askopenfilename(title="选择识别图片",filetypes=[("avi图片","*.avi"),["shipin","*.jpg"]])
    ...
    def show_result_per(self, img, img_info):
        ...
        #print(type(pilImage))
        #print(pilImage.size)
        pilImage = pilImage.resize((pilImage.size[0], pilImage.size[1]), Image.ANTIALIAS)
        tkImage = ImageTk.PhotoImage(image=pilImage)
